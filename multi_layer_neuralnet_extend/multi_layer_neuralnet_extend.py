...
x_train, t_train = dataset.get_labeled_data() #行122列
...
network = MultiLayerNetExtend(input_size=122, hidden_size_list=[50,50], output_size=2,
                            activation='relu', weight_init_std=0.01,
                            weight_decay_lambda=0, use_dropout=True, dropout_ration=0.5,
                            use_batchnorm=False)

#ハイパーパラメータ
iters_num = 294265 #イテレーションの回数
batch_size = 128
...
for i in range(iters_num): #イテレーションの回数分for文を回している
    batch_mask = np.random.choice(train_size, batch_size) #batch_size分のbatch_size行1列の乱数を作成
